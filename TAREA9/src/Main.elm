...
main =
  Browser.sandbox { model = init, update = update, view = view }
...
updateDisplay model number = 
  if model.append then 
    {model | display = model.display ++ withString (number)}
  else 
    { model | display = number |> withString , append = True}
...
calcular model =
  model.function model.saveValue (parseFloat model.display) |> withString 
